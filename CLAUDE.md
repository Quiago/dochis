...
- **2026-09-22:** Verificación asistida en lugar de aprobación manual: los registros oficiales (DHA, MOHAP) tienen CAPTCHA y no hay API gratuita, así que no se automatiza la consulta. El perfil se publica con "Licencia DHA 12345 (declarada)" y un botón al registro oficial; la revisión automática y los reportes de la comunidad son la red de seguridad. Los reclamos solo son automáticos si el teléfono/correo coincide con la lista importada; si no, se crea un perfil nuevo y el importado con el mismo nombre se oculta.
- **Descartado:** consultar los registros oficiales con navegador automatizado o peticiones directas (saltarse CAPTCHA); API de pago de terceros (Surepass) por coste y privacidad.
