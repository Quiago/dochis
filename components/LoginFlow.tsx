...
import { Button, Flash, FormControl, Heading, Link, Spinner, TextInput } from '@primer/react'
import { CheckCircleIcon } from '@primer/octicons-react'

type Step =
  | { name: 'phone'; error?: string }
  | { name: 'code'; phone: string; code: string; link: string }
  | { name: 'expired' }
  | { name: 'done' }

export default function LoginFlow({ next, intro }: { next: string; intro: string }) {
  ...
  async function requestCode(e: React.FormEvent<HTMLFormElement>) {
    ...
    const res = await fetch('/api/auth/challenge', { method: 'POST', body: JSON.stringify({ phone }) }).catch(() => null)
    const data = await res?.json().catch(() => null)
    setBusy(false)
    if (!res?.ok || !data?.code) return setStep({ name: 'phone', error: data?.error ?? 'No pudimos conectar. Inténtalo de nuevo.' })
    setStep({ name: 'code', ...data })
    QRCode.toDataURL(data.link, { margin: 1, width: 184 }).then(setQr, () => setQr(undefined))
  }

  // Polls every 2 s while waiting for the WhatsApp message.
  useEffect(() => {
  ...
  }, [step.name, next, router])

  if (step.name === 'done') {
    return (
      <div className="center" role="status">
        <CheckCircleIcon size={32} className="success-icon" />
        <Heading as="h2" className="auth-title">Número verificado</Heading>
  ...
  if (step.name === 'expired') {
    return (
      <div className="center">
        ...
        <p className="muted">Los códigos duran 10 minutos. Pide uno nuevo.</p>
        <Button variant="primary" block onClick={() => setStep({ name: 'phone' })}>Pedir otro código</Button>
      </div>
    )
  }
  ...
  if (step.name === 'code') {
    return (
      <div>
        ...
        <p className="center small"><Link as="button" onClick={() => setStep({ name: 'phone' })}>Cambiar número</Link></p>
      </div>
    )
  }
  ...
  return (
    <form onSubmit={requestCode}>
      ...
      <FormControl required>
        ...
        <TextInput name="phone" type="tel" inputMode="tel" autoComplete="tel" defaultValue="+971 " block size="large" />
